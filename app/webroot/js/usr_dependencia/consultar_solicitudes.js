...
function buscar_solicitudes(frase_busqueda, criterio_fecha, fecha_1, fecha_2, mostrar_solicitudes, criterio_campo, tipo_servicio)
{
	...
	{
		type: "POST",
		url: '/solicitudes/usr_buscar/'+frase_busqueda+'/'+criterio_fecha+'/'+fecha_1+'/'+fecha_2+'/'+mostrar_solicitudes+'/'+criterio_campo+'/'+tipo_servicio,
		dataType: 'json',
		cache: false,
		async: false,
...
function buscar_solicitud(id)
{
	...
	{
		type: "POST",
		url: '/solicitudes/usr_info_solicitud/'+id,
		dataType: 'json',
		cache: false,
		async: false,
		success: function(datos_json)
		{
			if ( datos_json.resultado == false )
			{
				$('#archivo_xls a').attr('href', '#');
				$('#archivo_xls').hide();
			}
...
{
	jQuery('#archivo_xls a').attr('href', '/solicitudes/usr_exportar_xls/'+frase_busqueda+'/'+criterio_fecha+'/'+fecha_1+'/'+fecha_2+'/'+mostrar_solicitudes+'/'+criterio_campo+'/'+tipo_servicio);
